...
class Entries extends CI_Controller {
	...
	public function __construct()
	{
	 ...
	 /**
	  *  @Description: test to add the matrix block
	  *       @Params: fieldName as ajax
	  *
	  *  	 @returns: json array
	  */
	public function add_matrix_block()
	{
	 ...
	 /**
	  *  @Description: save the order the multiples
	  *       @Params: params
	  *
	  *  	 @returns: returns
	  */
	...
	{
		$tmp =  trim($this->input->post('order'),',');
		...
		foreach ($tmp2 as $key) 
		{
			$object = array('sort_order' => $counter );

			$this->db->where('id', $key);
			$this->db->update('entry', $object);
	...
	public function show_multiple_view($sectionid)
	{
		...
		$this->load->view('admin/entries/footer-m');
