...
class Stuff_fields extends CI_Model {
	...
	public function add_new_field($handle,$type,$opts,$instructions,$maxchars,$limitamount,$formvalidation,$min,$max)
	{
     ...
     /**
      *  @Description: check if field name does not conflict with system names!
      *       @Params: handle
      *
      *       @returns: TRUE or FALSE
      */
     public function isValid($handle)
     {
        $sysNames = array('field','content','url','section','id','sitename' );

        $pass = 1;
        foreach ($sysNames as $key) 
        {
             if ($key == $handle) 
             {
                    $pass = 0;
